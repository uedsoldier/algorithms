...
#define TEST_NAME "CRC8-EBU (without lookup table) tester"
...
static bool run_single_test(const TestInput *test, uint8_t expected_crc,
    ...
    // Get CRC8
    test_passed = test_crc8(test->input, test->input_len, expected_crc, CRC8_EBU);
...
int main(void) {
    ...
    assert(sizeof(expected_crc8_ebu_crcs) /
                   sizeof(expected_crc8_ebu_crcs[0]) ==
               TOTAL_TESTS &&
           "Expected CRC count must match test input count");

    printf("%s\n\n", TEST_NAME);
    crc8_print_macro_settings();
    print_crc8_config(CRC8_EBU);

    bool all_tests_passed = true;

    // Run all tests
    for (size_t i = 0; i < TOTAL_TESTS; i++) {
        if (!run_single_test(&test_inputs[i], expected_crc8_ebu_crcs[i], i)) {
