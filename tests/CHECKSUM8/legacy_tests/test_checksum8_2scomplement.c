...
const char *TEST_NAME = "checksum8-2complement";
const CHECKSUM8_t checksum_type = CHECKSUM8_2complement;

const uint8_t expected_checksum8_2complement = 0x95;
...
int main(int argc, char *argv[])
{
    ...
    printf("Checksum %s: 0x%02X - Expected: 0x%02X\n", checksum8_implementations[checksum_type], checksum, expected_checksum8_2complement);

    printf("%s %s",TEST_NAME, checksum == expected_checksum8_2complement? "OK":"not OK");
    return checksum == expected_checksum8_2complement? EXIT_SUCCESS:EXIT_FAILURE;
}