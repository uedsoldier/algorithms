...
static bool run_single_test(const AES_TestInput *test, size_t test_number) {
    ...
    AES256_init_ctx(&state.ctx, test->key, test->init_vector);

    // Encrypt
    AES_errcode_t err_code =
        AES256_CBC_encrypt(&state.ctx, test->input_string, state.encrypt_buffer,
    ...
    err_code = AES256_CBC_decrypt(&state.ctx, state.encrypt_buffer,
