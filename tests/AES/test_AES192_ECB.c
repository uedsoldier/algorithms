...
static bool run_single_test(const AES_TestInput *test, size_t test_number) {
    ...
    AES192_init_ctx(&state.ctx, test->key, NULL);

    // Encrypt
    AES_errcode_t err_code =
        AES192_ECB_encrypt(&state.ctx, test->input_string, state.encrypt_buffer,
    ...
    err_code = AES192_ECB_decrypt(&state.ctx, state.encrypt_buffer,
