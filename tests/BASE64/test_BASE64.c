...
static const TestCase test_cases[] = {
    ...
    // Special character cases
    {
        ...
        .expect_dec = (const uint8_t *)"%",  // Decoding it back should give us '%'
    ...
    // Edge cases
    {
        ...
        .input = (const uint8_t *)" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~",
        .input_len = 95,
        .expect_enc = (const uint8_t *)"ICEiIyQlJicoKSorLC0uLzAxMjM0NTY3ODk6Ozw9Pj9AQUJDREVGR0hJSktMTU5PUFFSU1RVVldYWVpbXF1eX2BhYmNkZWZnaGlqa2xtbm9wcXJzdHV2d3h5ent8fX4=",
        .expect_enc_len = 128,
        .expect_dec = (const uint8_t *)" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~",
        .expect_dec_len = 95
    },
    {
    ...
    // Long string test
    {
        ...
        .input = (const uint8_t *)"The quick brown fox jumps over the lazy dog 1234567890 !@#$%^&*()",
        .input_len = 63,
        .expect_enc = (const uint8_t *)"VGhlIHF1aWNrIGJyb3duIGZveCBqdW1wcyBvdmVyIHRoZSBsYXp5IGRvZyAxMjM0NTY3ODkwICFAIyQlXiYqKCk=",
        .expect_enc_len = 84,
        .expect_dec = (const uint8_t *)"The quick brown fox jumps over the lazy dog 1234567890 !@#$%^&*()",
        .expect_dec_len = 63
    }
};
 ...
 */
static bool bytes_equal(const uint8_t *s1, size_t len1, const uint8_t *s2, size_t len2)
{
    if (len1 != len2)
        return false;
    if (s1 == NULL && s2 == NULL)
 ...
 */
static bool run_single_test(base64_converter_t *converter, const TestCase *test, size_t test_number)
{
    ...
    size_t encoded_size;
    base64_status_t status = base64_encode(converter,
                                           test->input,
                                           test->input_len,
                                           &encoded_data,
                                           &encoded_size);
    ...
    size_t decoded_size;
    status = base64_decode(converter,
                           encoded_data,
                           encoded_size,
                           decoded_buffer,
                           &decoded_size);
    ...
    if (!test_passed)
    {
        ...
        if (!encoding_correct)
        {
            printf("Expected Base64: '%.*s'\n", (int)test->expect_enc_len, test->expect_enc);
            printf("Actual Base64:   '%.*s'\n", (int)encoded_size, encoded_data);
        }
        if (!decoding_correct)
        {
            printf("Expected string: '%.*s'\n", (int)test->expect_dec_len, test->expect_dec);
