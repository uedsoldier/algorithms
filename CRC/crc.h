...
/**
 * @brief Macro para utilización de nombres de implementación de algoritmos CRC
*/
...
static const char *const crc8_implementations[] = {
	crc8_implementation_0, crc8_implementation_1, crc8_implementation_2, crc8_implementation_3, crc8_implementation_4, crc8_implementation_5,
	crc8_implementation_6, crc8_implementation_7, crc8_implementation_8, crc8_implementation_9, crc8_implementation_10, crc8_implementation_11,
	crc8_implementation_12
};

static const char *const crc16_implementations[] = {
	crc16_implementation_0, crc16_implementation_1, crc16_implementation_2, crc16_implementation_3, crc16_implementation_4, crc16_implementation_5,
	crc16_implementation_6, crc16_implementation_7, crc16_implementation_8, crc16_implementation_9, crc16_implementation_10, crc16_implementation_11,
	crc16_implementation_12, crc16_implementation_13, crc16_implementation_14, crc16_implementation_15, crc16_implementation_16, crc16_implementation_17,
	crc16_implementation_18, crc16_implementation_19, crc16_implementation_20, crc16_implementation_21, crc16_implementation_22
};

static const char *const crc32_implementations[] = {
	crc32_implementation_0, crc32_implementation_1, crc32_implementation_2, crc32_implementation_3, crc32_implementation_4, crc32_implementation_5,
	crc32_implementation_6, crc32_implementation_7, crc32_implementation_8//, crc32_implementation_9, crc32_implementation_10
};

static uint8_t crc8_implementations_size = sizeof(crc8_implementations);
static uint8_t crc16_implementations_size = sizeof(crc16_implementations);
static uint8_t crc32_implementations_size = sizeof(crc32_implementations);
...
#define CRC16_0x3D65_LOOKUP_TABLE       // 0x3D65 --> x^16 + x^13 + x^12 + x^11 + x^10 + x^8 + x^6 + x^5 + x^2 + 1
#define CRC16_0x8BB7_LOOKUP_TABLE       // 0x8BB7 --> x^16 + x^15 + x^11 + x^9 + x^8 + x^7 + x^5 + x^4 + x^2 + x + 1
#define CRC16_0xA097_LOOKUP_TABLE       // 0xA097 --> x^16 + x^15 + x^13 + x^7 + x^4 + x^2 + x^1 + 1
...
#define CRC32_0x04C11DB7_LOOKUP_TABLE // 0x04C11DB7 --> x^32 + x^26 + x^23 + x^22 + x^16 + x^12 + x^11 + x^10 + x^8 + x^7 + x^5 + x^4 + x^2 + x + 1
#define CRC32_0x000000AF_LOOKUP_TABLE // 0x000000AF --> x^32 + x^7 + x^5 + x^3 + x^2 + x + 1
//#define CRC32_0x32583499_LOOKUP_TABLE       // 0x32583499 --> x^32 + x^29 + x^28 + x^25 + x^22 + x^20 + x^19 + x^13 + x^12 + x^10 + x^7 + x^4 + x^3 + 1
//#define CRC32_0x741B8CD7_LOOKUP_TABLE       // 0x741B8CD7 --> x^32 + x^30 + x^29 + x^28 + x^26 + x^20 + x^19 + x^17 + x^16 + x^15 + x^11 + x^10 + x^7 + x^6 + x^4 + x^2 + x + 1
#endif
 ...
 */
static const uint8_t CRC8_0x07_table[256] = {};
#endif
...
/**
 * @brief Polinomio generador: 0x3D65 --> x^16 + x^13 + x^12 + x^11 + x^10 + x^8 + x^6 + x^5 + x^2 + 1
 */
...
/**
 * @brief Polinomio generador: 0x8BB7 --> x^16 + x^15 + x^11 + x^9 + x^8 + x^7 + x^5 + x^4 + x^2 + x + 1
 */
...
/**
 * @brief Polinomio generador: 0xA097 --> x^16 + x^15 + x^13 + x^7 + x^4 + x^2 + x^1 + 1
 */
...
/**
 * @brief Polinomio generador: 0xC867 --> x^16 + x^15 + x^14 + x^11 + x^6 + x^5 + x^2 + x^1 + 1
 */
...
/**
 * @brief Polinomio generador: 0xA833982B --> x^32 + x^31 + x^29 + x^27 + x^21 + x^20 + x^17 + x^16 + x^15 + x^12 + x^11 + x^5 + x^3 + x + 1
 */
...
/**
 * @brief Polinomio generador: 0x814141AB --> x^32 + x^31 + x^24 + x^22 + x^16 + x^14 + x^8 + x^7 + x^5 + x^3 + x + 1
 */
...
/**
 * @brief Polinomio generador: 0x1EDC6F41 --> x^32 + x^28 + x^27 + x^26 + x^25 + x^23 + x^22 + x^20 + x^19 + x^18 + x^14 + x^13 + x^11 + x^10 + x^9 + x^8 + x^6 + 1
 */
...
/**
 * @brief Polinomio generador: 0x04C11DB7 --> x^32 + x^26 + x^23 + x^22 + x^16 + x^12 + x^11 + x^10 + x^8 + x^7 + x^5 + x^4 + x^2 + x + 1
 */
...
/**
 * @brief Polinomio generador: 0x000000AF --> x^32 + x^7 + x^5 + x^3 + x^2 + x + 1
 */
...
/**
 * @brief Polinomio generador: 0x32583499 --> x^32 + x^29 + x^28 + x^25 + x^22 + x^20 + x^19 + x^13 + x^12 + x^10 + x^7 + x^4 + x^3 + 1
 */
...
/**
 * @brief Polinomio generador: 0x741B8CD7 --> x^32 + x^30 + x^29 + x^28 + x^26 + x^20 + x^19 + x^17 + x^16 + x^15 + x^11 + x^10 + x^7 + x^6 + x^4 + x^2 + x + 1
 */
...
// Funciones de obtención de polinomios generadores de CRC para 8, 16 y 32 bits.
...
// Funciones de obtención de valores iniciales (semilla) de CRC para 8, 16 y 32 bits.
...
// Función para verificación de valor de entrada reflejada (input reflected) de CRC para 8, 16 y 32 bits.
static bool CRC_getInputReflected(crc_t crc_type);

// Función para verificación de valor de salida reflejada (output reflected) de CRC para 8, 16 y 32 bits.
static bool CRC_getOutputReflected(crc_t crc_type);

// Funciones para obtención de valor de máscara XOR de valor de salida de CRC para 8, 16 y 32 bits.
