...
const uint8_t* p_tabla;   
   ...
   bool inputReflected = CRC_getInputReflected(crc_type);
   bool outputReflected = CRC_getOutputReflected(crc_type);
   while(dataLength-- != 0 )
   {
      uint8_t b = *_buf++;
      b = inputReflected? invierte_bitsByte(b) : b;
   ...
   crc = outputReflected? invierte_bitsByte(crc) : crc;
...
uint16_t CRC16(void *data, uint16_t dataLength, crc_t crc_type)
{
   ...
   crc = outputReflected? invierte_bitsInt16(crc) : crc;
...
uint32_t CRC32(void *data, uint16_t dataLength, crc_t crc_type)
{
   ...
   crc = outputReflected? invierte_bitsInt32(crc) : crc;
