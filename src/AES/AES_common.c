...
void MixColumns(uint8_t *plain_text) {
    ...
    for (size_t i = 0; i != AES_BLOCK_LEN; i += 4) {
        temp_block[i] = gmul(plain_text[i], (uint8_t)2) ^
                        gmul(plain_text[i + 1], (uint8_t)3) ^
                        plain_text[i + 2] ^ plain_text[i + 3];
        temp_block[i + 1] =
            plain_text[i] ^ gmul(plain_text[i + 1], (uint8_t)2) ^
            gmul(plain_text[i + 2], (uint8_t)3) ^ plain_text[i + 3];
        temp_block[i + 2] = plain_text[i] ^ plain_text[i + 1] ^
                            gmul(plain_text[i + 2], (uint8_t)2) ^
                            gmul(plain_text[i + 3], (uint8_t)3);
        temp_block[i + 3] = gmul(plain_text[i], (uint8_t)3) ^
                            plain_text[i + 1] ^ plain_text[i + 2] ^
                            gmul(plain_text[i + 3], (uint8_t)2);
    }
...
void ReverseMixColumns(uint8_t *plain_text) {
    ...
    for (i = 0; i != AES_BLOCK_LEN; i += 4) {
        ...
        temp_block[i] = gmul(plain_text[i], (uint8_t)14) ^
                        gmul(plain_text[i + 1], (uint8_t)11) ^
                        gmul(plain_text[i + 2], (uint8_t)13) ^
                        gmul(plain_text[i + 3], (uint8_t)9);

        temp_block[i + 1] = gmul(plain_text[i], (uint8_t)9) ^
                            gmul(plain_text[i + 1], (uint8_t)14) ^
                            gmul(plain_text[i + 2], (uint8_t)11) ^
                            gmul(plain_text[i + 3], (uint8_t)13);

        temp_block[i + 2] = gmul(plain_text[i], (uint8_t)13) ^
                            gmul(plain_text[i + 1], (uint8_t)9) ^
                            gmul(plain_text[i + 2], (uint8_t)14) ^
                            gmul(plain_text[i + 3], (uint8_t)11);

        temp_block[i + 3] = gmul(plain_text[i], (uint8_t)11) ^
                            gmul(plain_text[i + 1], (uint8_t)13) ^
                            gmul(plain_text[i + 2], (uint8_t)9) ^
                            gmul(plain_text[i + 3], (uint8_t)14);
    }
